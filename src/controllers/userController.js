...
// console.log(emailId,password)
  let savedData = await userModel.findOne({ password: password, emailId: emailId })
  // console.log(savedData)
  if (!savedData) return res.send({ status: false, msg: "this is not match password and emailId" })
  let token = jwt.sign(
    {
    ...
    "functionup-lithiium-very-very-secret-key"
  );
...
const getUserData = async function(req ,res){
  ...
  let decodedToken = jwt.verify(token, "functionup-lithiium-very-very-secret-key")
...
const deletUser = async function(req ,res){
  ...
  if (!decodedToken)
    ...
    let  userId = req.params.userId
    let Check = await userModel.findOneAndUpdate({ _id: userId }, {$set : {isDeleted: "true"}},{new :true})
