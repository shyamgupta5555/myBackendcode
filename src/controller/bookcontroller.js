...
const createbooks = async (req, res) => {
    try {
        const data = req.body;
        const { title, excerpt, userId, ISBN, category, subcategory, isDelete, releasedAt } = data;
        ...
        if (!userid) return res.status(404).send({ status: false, message: "user is not found" });
...
const getbooks = async function (req, res) {
    try {
        ...
        const data = await booksModel
            .find(input).select({ title: 1, excerpt: 1, userId: 1, category: 1, releasedAt: 1, reviews: 1, }).sort({ title: 1 });
...
const getBybookid = async (req, res) => {
    try {
        ...
        if (reviews.length == 0) return res.status(200).send({
            status: true,
            msg: "#book-details-response-no-reviews", data: data,
        });
